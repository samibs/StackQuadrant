...
import { createSuggestion, checkAndApplyCommunityVerification, getRegisteredSite } from "@/lib/db/queries";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(`gateway:suggest:${siteId}:${ip}`, 5, 3600000);
    if (!rl.success) {
    ...
    if (body.evidenceLinks !== undefined && body.evidenceLinks !== null) {
      if (!Array.isArray(body.evidenceLinks)) {
        ...
        for (let i = 0; i < body.evidenceLinks.length; i++) {
          ...
          else if (body.evidenceLinks[i]) evidenceLinks.push(body.evidenceLinks[i]);
        }
      }
    }
    ...
    if (body.tags !== undefined && body.tags !== null) {
      if (!Array.isArray(body.tags)) {
        ...
        for (const tag of body.tags) {
          if (typeof tag !== "string") { errors.push({ field: "tags", message: "Each tag must be a string" }); break; }
          tags.push(tag);
        }
      }
    }
    ...
    console.error("POST /api/v1/gateway/suggest error:", error);
