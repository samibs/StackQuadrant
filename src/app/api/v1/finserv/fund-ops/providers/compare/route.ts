...
export async function GET(request: NextRequest) {
  try {
    ...
    if (vendorIds.length < 2 || vendorIds.length > 5) {
      return apiError("VALIDATION_FAILED", "Provide 2-5 provider IDs separated by commas", 400);
    }

    const comparison = await getProviderComparison(userTeams[0].id, vendorIds);
    return apiSuccess(comparison);
  } catch (error) {
    console.error("GET /api/v1/finserv/fund-ops/providers/compare error:", error);
