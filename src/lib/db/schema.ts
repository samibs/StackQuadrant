...
// ============================================
// FinServ DORA-aligned incident intelligence
// Taxonomy weights and risk scores are StackQuadrant methodology, not ESA regulatory severity values.
// ============================================
...
export const doraIncidentCategories = pgTable("dora_incident_categories", {
  ...
  name: varchar("name", { length: 120 }).notNull(),
  description: text("description").notNull(),
  severityWeight: decimal("severity_weight", { precision: 3, scale: 2 }).notNull(),
  displayOrder: integer("display_order").notNull().default(0),
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
...
export const esaIncidentReports = pgTable("esa_incident_reports", {
  ...
  check("esa_reports_total_incidents_nonnegative", sql`${table.totalIncidents} >= 0`),
  check("esa_reports_period_order", sql`${table.reportPeriodStart} IS NULL OR ${table.reportPeriodEnd} IS NULL OR ${table.reportPeriodStart} <= ${table.reportPeriodEnd}`),
]);
...
export const vendorDoraIncidents = pgTable("vendor_dora_incidents", {
  ...
  check("vendor_dora_resolution_order", sql`${table.resolvedAt} IS NULL OR ${table.resolvedAt} >= ${table.occurredAt}`),
]);
...
export const vendorDoraRiskScores = pgTable("vendor_dora_risk_scores", {
  ...
  scoringModelVersion: varchar("scoring_model_version", { length: 40 }).notNull().default("sq-dora-v1"),
  ...
  check("vendor_dora_incident_count_nonnegative", sql`${table.incidentCount} >= 0`),
]);
